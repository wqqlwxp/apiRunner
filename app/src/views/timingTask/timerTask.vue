    ...
    <el-row >
        <el-row class="pading-20 my-border my-bg">
                <el-breadcrumb separator="/">
                    ...
                    <el-breadcrumb-item>定时维护</el-breadcrumb-item>
                </el-breadcrumb>
        </el-row>
        ...
        <el-row style="margin-top: 20px">
            <el-table :data="tableTaskData" style="width: 100%"  >
                <el-table-column  fixed="left" align="center" label="序号"  width="100">
                    <template  slot-scope="{$index}">
                        <span >{{(currentPage-1)*pageSize+$index+1}}</span>
                    </template>
                </el-table-column>
                <el-table-column v-for="(item,key) in tableScheduleTaskTitle" :key="key" :prop="item.prop" align="center" :label="item.label" :width="item.width">
...
<script>
    ...
    export default {
        ...
        data(){
            return{
                tableScheduleTaskTitle:templateData.tableScheduleTaskTitle,
                tableScheduleTaskInitData:templateData.tableScheduleTaskInitData,
                tableTaskData:[],
                currentPage:1,
                pageSize: 5,
                totalSize: 100,
        ...
        methods:{
            ...
            getData(offset,limit,project){
                ...
                post('api/task/queryAllTask',JSON.stringify(pageObject)).then((response)=> {
                            this.tableTaskData=response.data.rows;
            ...
            save(row,index){
                ...
                if(!isNotEmpty(row.projectCode) || !isNotEmpty(row.cron)){
                    this.$message({type: 'warning', message: '数据不能为空!'});
                    return
                }

                try{
                    this.$cronParser.parseExpression(row.cron);
                } catch (e) {
                    this.$message({type: 'warning', message: 'cron表达式错误，请确认!'});
                    return
                }

                row.operater=queryAuthName();
                this.changeFlag=false;
                post('api/task/editTask',JSON.stringify(row)).then((response)=> {
            ...
            remove(row){
                ...
                this.$confirm('此操作将删除该数据, 是否继续?', '提示', {
                    ...
                    post('api/task/deleteTask',JSON.stringify(row)).then((response)=> {
                        if(response.data.code=='200'){
                            this.currentPage=1;
                            this.getData(1,this.pageSize,this.queryProjectText);
                            this.$message({type: 'success', message: response.data.msg});
            ...
            add(){
                ...
                this.tableTaskData.unshift(JSON.parse(JSON.stringify(this.tableScheduleTaskInitData)));
