    ...
    <el-row >
        <el-row class="pading-20 my-border my-bg">
                <el-breadcrumb separator="/">
                    ...
                    <el-breadcrumb-item>数据库维护</el-breadcrumb-item>
                </el-breadcrumb>
        </el-row>
        ...
        <el-row style="margin-top: 20px">
            <el-table :data="tableDatabaseData" style="width: 100%"  >
                <el-table-column  fixed="left" align="center" label="序号" width="100" >
                    <template  slot-scope="{$index}">
                        <span >{{(currentPage-1)*pageSize+$index+1}}</span>
                    </template>
                </el-table-column>
                <el-table-column v-for="(item,key) in tableDatabaseTitle" :key="key" :prop="item.prop" align="center" :label="item.label" :width="item.width">
...
<script>
    ...
    export default {
        ...
        data(){
            return{
                tableDatabaseTitle:templateData.tableDatabaseTitle,
                tableDatabaseInitData:templateData.tableDatabaseInitData,
                tableDatabaseData:[],
                currentPage:1,
                pageSize: 5,
                totalSize: 100,
                ...
                moduleAllData:[],
                changeFlag:false,
                filterList:['status','projectCode','moduleCode'],
            }
        },
        methods:{
            ...
            getData(offset,limit,project,module){
                ...
                post('api/databaseConfig/queryAllDatabaseConfig',JSON.stringify(pageObject)).then((response)=> {
                            this.tableDatabaseData=response.data.rows;
            ...
            save(row,index){
                ...
                if(!isNotEmpty(row.connectionAddress) || !isNotEmpty(row.connectionUser)  || !isNotEmpty(row.connectionPwd) ){
                    this.$message({type: 'warning', message: '连接属性不能为空!'});
                    return
                }
                ...
                pageObject["data"]=row;
                post('api/databaseConfig/editDatabaseConfig',JSON.stringify(pageObject)).then((response)=> {
            ...
            remove(row){
                ...
                this.$confirm('此操作将删除该数据, 是否继续?', '提示', {
                    ...
                    post('api/databaseConfig/deleteDatabaseConfig',row.id).then((response)=> {
                        if(response.data.code=='200'){
                            this.currentPage=1;
                            this.getData(1,this.pageSize,this.queryProjectText,this.queryModuleText);
                            this.$message({type: 'success', message: response.data.msg});
            ...
            add(){
                ...
                this.tableDatabaseData.unshift(JSON.parse(JSON.stringify(this.tableDatabaseInitData)));
