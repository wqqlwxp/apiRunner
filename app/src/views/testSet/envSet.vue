    ...
    <el-row >
        <el-row class="pading-20 my-border my-bg">
                <el-breadcrumb separator="/">
                    ...
                    <el-breadcrumb-item>环境维护</el-breadcrumb-item>
                </el-breadcrumb>
        </el-row>
        ...
        <el-row style="margin-top: 20px">
            <el-table :data="tableEnvData" style="width: 100%"  >
                <el-table-column  fixed="left" align="center" label="序号" width="100" >
                    <template  slot-scope="{$index}">
                        <span >{{(currentPage-1)*pageSize+$index+1}}</span>
                    </template>
                </el-table-column>
                <el-table-column v-for="(item,key) in tableEnvTitle" :key="key" :prop="item.prop" align="center" :label="item.label" :width="item.width">
                ...
                <el-table-column  fixed="right" align="center" label="操作" width="200">
                    <template  slot-scope="{row,$index}">
                        <i v-per="'env_edit'"  class="el-icon-edit i_pry"    @click="change(row,$index,true)" ></i>
                        <i v-per="'env_save'"  class="el-icon-check i_pry"     @click="save(row,$index)" ></i>
                        <i v-per="'env_del'"   class="el-icon-delete i_pry"     @click="remove(row)" ></i>
                    </template>
                </el-table-column>
            </el-table>
...
<script>
    ...
    export default {
        ...
        data(){
            return{
                tableEnvTitle:templateData.tableEnvTitle,
                tableEnvInitData:templateData.tableEnvInitData,
                tableEnvData:[],
                currentPage:1,
                pageSize: 5,
                totalSize: 100,
        ...
        methods:{
            ...
            getData(offset,limit,project){
                ...
                post('api/env/queryAllEnvMachine',JSON.stringify(pageObject)).then((response)=> {
                            this.tableEnvData=response.data.rows;
            ...
            save(row,index){
                ...
                if(!isNotEmpty(row.projectCode) ||  !isNotEmpty(row.envCode) || !isNotEmpty(row.ip || !isNotEmpty(row.port))){
                ...
                pageObject["data"]=row;
                post('api/env/editEnvMachine',JSON.stringify(pageObject)).then((response)=> {
            ...
            remove(row){
                ...
                this.$confirm('此操作将删除该数据, 是否继续?', '提示', {
                    ...
                    post('api/env/deleteEnvMachine',JSON.stringify(row)).then((response)=> {
                        if(response.data.code=='200'){
                            this.currentPage=1;
                            this.getData(1,this.pageSize,this.queryProjectText);
                            this.$message({type: 'success', message: response.data.msg});
            ...
            add(){
                ...
                this.tableEnvData.unshift(JSON.parse(JSON.stringify(this.tableEnvInitData)));
